...
qrun -batch -access=rw+/. -uvmhome uvm-1.2 '-timescale' '1ns/1ns' -mfcu design.sv testbench.sv '-voptargs=+acc=npr' '+UVM_VERBOSITY=UVM_MEDIUM' '+UVM_TESTNAME=test_top_all_feature' '+uvm_set_type_override=apb_drv,apb_drv'  -do  " run -all; exit"  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip